...
const getCandidateAge = (candidate: CandidateProfile) =>
...
const getCandidateInterests = (candidate: CandidateProfile) => {
  if (typeof candidate.interests === 'string') {
    return candidate.interests.split(',').map((s) => s.trim()).filter(Boolean)
  }
  return Array.isArray(candidate.interests) ? candidate.interests : []
}

const getCandidateImage = (candidate: CandidateProfile) =>
  candidate.profile_image_url || candidate.photo_url || candidate.image_url || candidate.Picture || ''
...
export default function Dashboard() {
  ...
  const handlePass = (candidateId: string) => {
    ...
    setCandidates(prev =>
      prev.filter(c => getCandidateId(c) !== String(candidateId))
    )
  }
      ...
      <main className="dashboard-content">
        <section className="welcome-section discover-hero">
          <div>
            ...
            <p>Review your best match recommendation and ignite the connection when it feels right.</p>
          </div>
                  ...
                  <div className="spotlight-image">
                    {currentImage ? (
                      ...
                      <div className="card-image-placeholder">
                        <span className="avatar-initial">{currentName.charAt(0).toUpperCase()}</span>
                      </div>
                    )}
                    <div className="spotlight-overlay">
                      <div>
                        <h3>{currentName}{currentAge ? `, ${currentAge}` : ''}</h3>
